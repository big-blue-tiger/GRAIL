...
class TRLPPOTrainer(PPOTrainer):  # noqa: F405
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            restore_env_state: Restore the checkpointed environment state during
                a full resume. Disable this when resuming onto a new dataset.
    ...
    def _init_config(self):
        ...
        self.dagger_mixed_rollout = bool(
            self.config.get("use_dagger", False)
            and "dagger_rollout_mode" in self.config
            and float(self.config.get("dagger_student_ratio", 0.5)) < 1.0
        )
    ...
    def _rollout_step(self, model, obs_dict):
        ...
        if self.dagger_mixed_rollout:
            ...
            if self._last_dagger_num_teacher > 0:
    ...
    def _get_rollout_data(self, obs_keys):
        ...
        if getattr(self.storage, "loss_teacher_termination_mask", None) is not None:
            mask = self.storage.query_key("loss_teacher_termination_mask").transpose(0, 1).to(device)
            rollout_data["loss_teacher_termination_mask"] = mask.repeat(2, 1) if self.use_symmetry else mask
        return rollout_data
    ...
    def _get_mb_rollout_data(self, rollout_data, micro_batch_inds):
        ...
        if "loss_teacher_termination_mask" in rollout_data:
            mb_rollout_data["loss_teacher_termination_mask"] = rollout_data["loss_teacher_termination_mask"][micro_batch_inds]
    ...
    def load_checkpoint(
        ...
        Args:
            ...
            restore_env_state: Whether to restore checkpointed environment state
                during a full resume. Trainer counters and schedules are restored
        ...
        if resume and not warm_resume:
            ...
            if restore_env_state and "env_state_dict" in checkpoint:
